...
const validation = yup.object().shape({
  id_patient: yup.number().required("ID do paciente é obrigatório."),
  id_nurse: yup.number().required("ID do enfermeiro é obrigatório."),
  nameMedication: yup
    .string()
  ...
  status: yup.boolean().default(false).required("Status é obrigatório"),
});

function validateCreateMedication(req, res, next) {
  try {
    const { body } = req;
